/*
 * Nova based ARM Versatile Express emulation.
 *
...
#define NOVA_BOARD_ID 0x8e0
#define NOVA_FLASH_SIZE (64 * 1024 * 1024)
#define NOVA_FLASH_SECT_SIZE (256 * 1024)
...
#define TYPE_NOVA_MACHINE   "nova"
#define TYPE_NOVA_15_MACHINE   MACHINE_TYPE_NAME("nova-15")
#define NOVA_MACHINE(obj) \
    OBJECT_CHECK(VexpressMachineState, (obj), TYPE_NOVA_MACHINE)
#define NOVA_MACHINE_GET_CLASS(obj) \
    OBJECT_GET_CLASS(VexpressMachineClass, obj, TYPE_NOVA_MACHINE)
#define NOVA_MACHINE_CLASS(klass) \
    OBJECT_CLASS_CHECK(VexpressMachineClass, klass, TYPE_NOVA_MACHINE)
...
static void nova_modify_dtb(const struct arm_boot_info *info, void *fdt)
{
                                             ...
                                             DriveInfo *di)
{
    ...
    qdev_prop_set_uint32(dev, "num-blocks",
                         NOVA_FLASH_SIZE / NOVA_FLASH_SECT_SIZE);
    qdev_prop_set_uint64(dev, "sector-length", NOVA_FLASH_SECT_SIZE);
...
static void nova_common_init(MachineState *machine)
{
    VexpressMachineState *vms = NOVA_MACHINE(machine);
    VexpressMachineClass *vmc = NOVA_MACHINE_GET_CLASS(machine);
    ...
    daughterboard->bootinfo.board_id = NOVA_BOARD_ID;
    daughterboard->bootinfo.loader_start = daughterboard->loader_start;
    daughterboard->bootinfo.smp_loader_start = map[VE_SRAM];
    daughterboard->bootinfo.smp_bootreg_addr = map[VE_SYSREGS] + 0x30;
    daughterboard->bootinfo.gic_cpu_if_addr = daughterboard->gic_cpu_if_addr;
    daughterboard->bootinfo.modify_dtb = nova_modify_dtb;
...
static bool nova_get_secure(Object *obj, Error **errp)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);

    return vms->secure;
}

static void nova_set_secure(Object *obj, bool value, Error **errp)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);
...
static bool nova_get_virt(Object *obj, Error **errp)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);
...
static void nova_set_virt(Object *obj, bool value, Error **errp)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);
...
static void nova_instance_init(Object *obj)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);

    /* EL3 is enabled by default on vexpress */
    vms->secure = true;
    object_property_add_bool(obj, "secure", nova_get_secure,
                             nova_set_secure, NULL);
...
static void nova_15_instance_init(Object *obj)
{
    VexpressMachineState *vms = NOVA_MACHINE(obj);
     ...
     */
    vms->virt = true;
    object_property_add_bool(obj, "virtualization", nova_get_virt,
                             nova_set_virt, NULL);
...
static void nova_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);

    mc->desc = "Nova based ARM Versatile Express";
    mc->init = nova_common_init;
    mc->max_cpus = 4;
    mc->ignore_memory_transaction_failures = true;
}

static void nova_15_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);
    VexpressMachineClass *vmc = NOVA_MACHINE_CLASS(oc);

    mc->desc = "Nova based on ARM Versatile Express for Cortex-A15";
    mc->default_cpu_type = ARM_CPU_TYPE_NAME("cortex-a15");

    vmc->daughterboard = &a15_daughterboard;
}

static const TypeInfo nova_info = {
    .name = TYPE_NOVA_MACHINE,
    .parent = TYPE_MACHINE,
    .abstract = true,
    .instance_size = sizeof(VexpressMachineState),
    .instance_init = nova_instance_init,
    .class_size = sizeof(VexpressMachineClass),
    .class_init = nova_class_init,
};

static const TypeInfo nova_15_info = {
    .name = TYPE_NOVA_15_MACHINE,
    .parent = TYPE_NOVA_MACHINE,
    .class_init = nova_15_class_init,
    .instance_init = nova_15_instance_init,
};

static void nova_machine_init(void)
{
    type_register_static(&nova_info);
    type_register_static(&nova_15_info);
}

type_init(nova_machine_init);
